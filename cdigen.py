...
import scripts.underwayweb,scripts.met_script, scripts.ts_script, scripts.sbe_script, scripts.generalweb, scripts.xbt ,scripts.adcp, scripts.ffe ,scripts.mbe, scripts.mcs, scripts.mag, scripts.sss, scripts.srs, scripts.sbp, scripts.net, scripts.cor, scripts.arg
...
import scripts.flo, scripts.dft, scripts.flu,scripts.rad
import csrcodelist
import requests
import shutil
import logging
...
def download_file():
    try:
        ...
        url_bbox = f"http://datahub.utm.csic.es/ws/getBBox/?id={vessel_reduit}{cruise_id[4:12]}"
        r = requests.get(url_bbox)
        print(f'Fetching bounding box from URL: {url_bbox}')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if filename is not None:
                try:
                    ruta_csv = "./static/csv/<filename>".replace("<filename>", filename)
            ...
            # Get instruments from the new hidden field (comma-separated values)
            selects_str = request.values.get('selects')
            logging.info(f"Raw selects value received: {selects_str}")
            logging.info(f"All request values: {request.values}")
            
            if not selects_str:
                logging.error("No selects field found in request")
                return jsonify({"error": "No instrument types selected"}), 400
            
            # Split the comma-separated values into a list
            selects = [instrument.strip() for instrument in selects_str.split(',') if instrument.strip()]
            
            logging.info(f"Parsed instruments: {selects}")
            
            if not selects:
                logging.error("Instruments list is empty after parsing")
                return jsonify({"error": "No instrument types selected"}), 400
