...
def cdi_general (cruise_id, cruise_name, date_inicial, date_final, vessel_input, data, valor_org ,csr_code):
  ...
  if path.exists("model_general.txt"):
    remove("model_general.txt")
  ...
  general_ctd =nombre_carpeta + "/" +cruise_id + "_ctd.xml"

  #if path.exists(underway_general):
    #remove(underway_general)
  if path.exists(general_ctd):
    remove(general_ctd)


  shutil.copy("model_general.xml", cdi_general)
  
#distingir quins cdi calen amb gml o quins no
  #Posem la url perque trobi el gml i l'enganxi en el xml
  input_file= cdi_general
  input_url='http://datahub.utm.csic.es/ws/getTrack/GML/?id='+ vessel_input+ cruise_id[4:12]+'&n=999'
  output_file= cdi_general
