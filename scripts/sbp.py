...
def funcio_sbp (cruise_id, cruise_name, date_inicial, date_final, vessel_input):
    ...
    underway_sbp =cruise_id + "/" + cruise_id + "_sbp.xml"
    ...
    shutil.copy(underway_general, underway_sbp)
    input_file= underway_sbp
    output_file= underway_sbp
    ...
    posList.text = cruise_name + " sub-bottom profiler data"
    tree.write(output_file)

    #afegir ABSTRACT
    tree = etree.parse(input_file)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Sub-bottom profiler data acquired on board the R/V "+ vessel + " with a Parasound P-35 parametric echosounder during the "+cruise_name+" cruise."
    tree.write(output_file)
