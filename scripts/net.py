...
def funcio_net (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_net"
...
#canviar paràmetres
    ...
    posList_1.text =  'Zooplankton biovolume'
    posList_1.set ("codeListValue","ZV00")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Zooplankton taxonomy-related abundance per unit volume of the water column'
    posList_2.set ("codeListValue","ZATX")
    posList= tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList.text =  'Plankton abundance per unit volume of the water column'
    posList.set ("codeListValue","PYTT")
    ...
    posList_1.text =  'plankton nets'
    posList_1.set ("codeListValue","22")
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_net.xml"
    ...
    if path.exists(cruise_id + "_net.txt"):
      remove(cruise_id + "_net.txt")

    #canviar segons el cdi:
    select_instrument = "RED"
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
        name=str(samples.loc [i,"index"])
        name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
        net_text = str(samples.loc [i,"Coments"])
        name3= "Biological samples acquired on board R/V " + vessel + "with a " +net_text+ " during the "+cruise_name+ " cruise."
        fila=fila+1
    ...
    posList.text = cruise_name + " net data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = str(total_lines) +" biological samples acquired on board R/V "+ vessel + " during the " +cruise_name+" cruise."
