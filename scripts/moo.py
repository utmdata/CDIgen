...
def funcio_moo (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_moo"
    ...
    posList_1.text =  'Sediment grain size parameters'
    posList_1.set ("codeListValue","MNGS")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Sedimentation flux quantification in the water column'
    posList_2.set ("codeListValue","MSFX")
    posList_3 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_3.text =  'Horizontal velocity of the water column (currents)'
    posList_3.set ("codeListValue","RFVL")
    posList_4 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_4.text =  'Vertical velocity of the water column (currents)'
    posList_4.set ("codeListValue","LRZA")
    ...
    posList_1.text =  'sediment traps'
    posList_1.set ("codeListValue","33")
    posList_2 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_2.text =  'current profilers'
    posList_2.set ("codeListValue","115")
    posList_3 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_3.text =  'current meters'
    posList_3.set ("codeListValue","114")
    posList_4 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_4.text =  'ADVs and turbulence probes'
    posList_4.set ("codeListValue","384")
    tree.write(output_file)
    ...
    posList.text = "mooring" 
    posList.set ("codeListValue","48")
    tree.write(output_file)
    ...
    for i in range(0,total_lines):
      ...
      name=str(samples.loc [i,"index"])
      name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
      name3= "Data from mooring acquired on board the R/V " + vessel + " during the " + cruise_name + " cruise."
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = "Data from " + str(total_lines) +  " moorings deployed on board the R/V "+ vessel +" during the " + cruise_name +" cruise."
