...
def funcio_obs (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_obs"
    ...
    posList_1.text =  'Active seismic refraction'
    posList_1.set ("codeListValue","ACSR")
    ...
    posList_1.text =  'hydrophones'
    posList_1.set ("codeListValue","369")
    posList_2 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_2.text =  'seismic refraction systems'
    posList_2.set ("codeListValue","155")
    tree.write(output_file)
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_obs.xml"
    ...
    if path.exists(cruise_id + "_obs.txt"):
      remove(cruise_id + "_obs.txt")

    #canviar segons el cdi:
    select_instrument = "OBS"
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name3= "Seismic data acquired with an OBS deployed during the " +cruise_name +" cruise on board the R/V  "+ vessel +"." #canviar  
      fila=fila+1
    ...
    posList.text = cruise_name + " OBS data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Seismic data acquired with " + str(total_lines) +" OBS deployed during the " +cruise_name +" cruise on board the R/V  "+ vessel +"." 
