...
def funcio_arg (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_arg"
    ...
    posList_1.text =  'Lagrangian currents and transport rates in the water column'
    posList_1.set ("codeListValue","LGCR")
    ...
    posList_3.text=  'Salinity of the water column'
    posList_3.set ("codeListValue","PSAL")
    tree.write(output_file)

    #canviar intruments 
    tree = etree.parse(input_file)
    posList_1 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_1.text =  'Argos GPS-localised transmitters'
    posList_1.set ("codeListValue","POS16")
    tree.write(output_file)
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_arg.xml"
    ...
    if path.exists(cruise_id + "_arg.txt"):
      remove(cruise_id + "_arg.txt")

    #canviar segons el cdi:
    select_instrument = "ARGO"
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name3= "Water column salinity and temperature data and lagrangian currents data aquired with ARGO floats during the " +cruise_name +" cruise on board the R/V "+ vessel +"." #canviar  
      fila=fila+1
    ...
    posList.text = cruise_name + " ARGO data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Water column salinity and temperature data and lagrangian currents data aquired with " + str(total_lines) +" ARGO floats during the " +cruise_name +" cruise on board the R/V  "+ vessel +"." 
