...
def funcio_flo (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_flo"
    ...
    posList_1.text =  'Dissolved oxygen parameters in the water column'
    posList_1.set ("codeListValue","DOXY")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Alkalinity, acidity and pH of the water column'
    posList_2.set ("codeListValue","ALKY")
    posList= tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList.text =  'Phosphate concentration parameters in the water column'
    posList.set ("codeListValue","PHOS")
    ...
    posList_1.text =  'discrete water samplers'
...
#<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name=str(samples.loc [i,"index"])
      name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
      name3= "Water samples data acquired on board the R/V " + vessel + " with a Go-Flo water sampler during the " + cruise_name + " cruise."
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = "Water samples data from "+ str(total_lines) +  " Go-Flo water sampler " + "acquired on board the R/V " + vessel + " during the " + cruise_name + " cruise."
