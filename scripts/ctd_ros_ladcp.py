...
def funcio_ctd_ros_ladcp (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_ctd_ros_ladcp"
    text = "CTD, rosette and LADCP" 
    fila=0
    lista_id=[]
    lista_name=[]
    lista_abstract=[]
    ...
    posList_1 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_1.text =  'particle sizers'
    posList_1.set ("codeListValue","150")

    posList_1 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_1.text =  'lowered current profilers'
    posList_1.set ("codeListValue","116")    
    tree.write(output_file)
    ...
    select_instrument = "CTD_ROS_LADCP"
...
#<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = "Water column data from "+ str(total_lines) +  " " + text  + " acquired on board the R/V " + vessel + " with a SeaBird SBE911 plus " + text + " during the " + cruise_name + " cruise."
