...
def funcio_isp (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_isp"
...
#canviar paràmetres
    ...
    posList_1.text =  'Molecular biology parameters'
    posList_1.set ("codeListValue","RBHY")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Biological detritus in the water column suspended particulate material'
    posList_2.set ("codeListValue","EXUV")
    posList= tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList.text =  'Other physical and chemical properties of suspended particulate material'
    posList.set ("codeListValue","DENS")
    ...
    posList_1.text =  'discrete water samples'
    posList_1.set ("codeListValue","30")
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_isp.xml"
    ...
    if path.exists(cruise_id + "_isp.txt"):
      remove(cruise_id + "_isp.txt")

    #canviar segons el cdi:
    select_instrument = "INSITUPUMPS"
    ...
    lista_instru=[]
    for i in range(0,total_lines):          
      instrument= 'insitu pump'
      lista_instru.append(instrument)
    samples['instrument'] = lista_instru 
    ...
    for i in range(0,total_lines):
      name=str(samples.loc [i,"index"])
      name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
      name3= "Data from a water sampler acquired on board R/V " + vessel + "with an insitupumps during the "+cruise_name+ " cruise."
      fila=fila+1
    ...
    posList.text = cruise_name + " insitupumps data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Data from " + str(total_lines) +" water samplers acquired on board R/V "+ vessel + " with the insitupumps during the " +cruise_name+" cruise."
