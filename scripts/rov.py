...
def funcio_rov (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_rov"
...
#canviar paràmetres
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_rov.xml"
    ...
    if path.exists(cruise_id + "_rov.txt"):
      remove(cruise_id + "_rov.txt")

    #canviar segons el cdi:
    select_instrument = "ROV"
    ...
    lista_instrument=[]
    for i in range(0,total_lines):    
      instrument= 'Remotely operated vehicle'
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name3= "ROV dive data from INSERT ROV acquired during the " +cruise_name+" cruise on board the R/V " + vessel + "."
      fila=fila+1
    ...
    print("datefinal",lista_date_final)
    ...
    posList.text = "ROV dive data from " + str(total_lines) +" INSERT ROV acquired during the " +cruise_name+" cruise on board the R/V " + vessel + "."
