...
def eliminar_columnes(csv_name):
  ...
  arxiu = arxiu.reindex(columns=["cruise_id","longitude", "latitude","Instrument","vessel", "id", "met_cat","parameter", "parameter_"])
  arxiu = arxiu.rename (columns={'longitude': 'lon', 'latitude': 'lat', 'Instrument': 'instrument','cruise_id': 'cruiseid','id': 'codiid'})
...
def funcio_svp (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_svp.xml"
    ...
    if path.exists(cruise_id + "_svp.txt"):
      remove(cruise_id + "_svp.txt")

    #canviar segons el cdi:   

    select_instrument = "SVP"
    ...
    for i in range(0,total_lines):
      ...
      name3= "Water column data from SVP launched on board the R/V "+ vessel +" during the " + cruise_name +" cruise." #canviar  
      fila=fila+1
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = cruise_name + " SVP data"
    tree.write(cdi_global)

    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Water column data from "+ str(total_lines) +" SVP launched on board the R/V "+ vessel +" during the " + cruise_name +" cruise."
    tree.write(cdi_global)
    print(total_lines)
