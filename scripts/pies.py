...
def funcio_pies (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_pies"
    ...
    posList_1.text =  'Sea level'
    posList_1.set ("codeListValue","ASLV")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Sound velocity and travel time in the water column'
    ...
    posList_1.text =  'inverted echosounders'
    posList_1.set ("codeListValue","112")
    tree.write(output_file)
...
#<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name=str(samples.loc [i,"index"])
      name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
      name3= "Water column data acquired with a Pressure Inverted Echo Sounder on board the R/V " + vessel +" during the " + cruise_name + " cruise."
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = "Water column data from "+ str(total_lines) +  " SVP acquired with a Pressure Inverted Echo Sounder on board the R/V " + vessel +" during the " + cruise_name + " cruise."
    tree.write(cdi_global)
    print(total_lines)
