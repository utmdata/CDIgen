...
def funcio_msc (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_msc"
...
#canviar paràmetres
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_msc.xml"
    ...
    if path.exists(cruise_id + "_msc.txt"):
      remove(cruise_id + "_msc.txt")

    #canviar segons el cdi:
    select_instrument = "MARINE SNOW CATCHER"
    ...
    lista_instrument=[]
    for i in range(0,total_lines):
      instrument= 'Marine Snow Catcher'
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      text = " marine snow catcher "  #canviar  
    ...
    for i in range(0,total_lines):
      ...
      name3= "Data from a water sampler acquired on board R/V " + vessel + "with a marine snow catcher during the "+cruise_name+ " cruise."
      fila=fila+1
    ...
    posList.text = cruise_name + " marine snow catcher data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = "Data from " + str(total_lines) +" water samplers acquired on board R/V "+ vessel + " with the marine snow catcher during the " +cruise_name+" cruise."
