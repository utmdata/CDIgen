...
def funcio_dft (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_dft"
    ...
    posList_1.text =  'Raw temperature and/or salinity instrument output'
    posList_1.set ("codeListValue","TCNT")
    posList_2 = tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList_2.text =  'Electrical conductivity of the water column'
    posList_2.set ("codeListValue","CNDC")
    posList= tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList.text =  'Air pressure'
    posList.set ("codeListValue","CAPH")
    posList= tree.xpath("//sdn:SDN_ParameterDiscoveryCode[contains(text(), 'Date and time')]", namespaces=namespace)[0]
    posList.text =  'Wind strength and direction'
    posList.set ("codeListValue","EWSB")
    ...
    posList_1.text =  'water pressure sensors'
    posList_1.set ("codeListValue","WPS")

    posList_1 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_1.text =  'satellite positioning systems'
    posList_1.set ("codeListValue","301")

    posList_1 = tree.xpath("//sdn:SDN_DeviceCategoryCode[contains(text(), 'unknown')]", namespaces=namespace)[0]
    posList_1.text =  'water body temperature sensor'
    posList_1.set ("codeListValue","302")
...
#<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
      ...
      name=str(samples.loc [i,"index"])
      name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
      name3= "Meteorological and oceanographic data acquired by drifting buoys deployed on board the R/V " + vessel + " during the " + cruise_name + " cruise."
  ...
  #afegir dataset id (ho fem tres cops perque s'ha de canviar tres vegades)
    ...
    posList.text = "Meteorological and oceanographic data acquired by "+ str(total_lines) +  " drifting buoys deployed on board the R/V " + vessel + " during the " + cruise_name + " cruise."
