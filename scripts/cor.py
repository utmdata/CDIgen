...
def funcio_cor (cruise_id, cruise_name, vessel_input, ruta_csv, date_inicial, date_final):
    cdi_model = "_cor"
...
#canviar paràmetres
    ...
    posList_1.text =  'Inorganic chemical composition of sediment or rock'
    posList_1.set ("codeListValue","RMIN")
    ...
    posList.text =  'Sediment grain size parameters'
    posList.set ("codeListValue","MNGS")
    ...
    posList_1.text =  'Unconsolidated sediment corers'
    posList_1.set ("codeListValue","51")
    ...
    cdi_global=cruise_id + "/" + cruise_id + "_cor.xml"
    ...
    if path.exists(cruise_id + "_cor.txt"):
      remove(cruise_id + "_cor.txt")

    #canviar segons el cdi:
    select_instrument = "CORER"
    ...
    for i in range(0,total_lines):          
      instrument2= 'unconsolidated sediment corer'
      lista_instru.append(instrument2)
  ...
  #<a href="http://data.utm.csic.es/geonetwork/srv/eng/catalog.search#/metadata/urn:SDN:CDI:LOCAL:29SG20230719_ctd_ros_ladcp"  target="_blank">View in metadata catalog</a>
    ...
    for i in range(0,total_lines):
        name=str(samples.loc [i,"index"])
        name = name.zfill(2) #fem que el nom sigui de 2 digits i ho ompli amb 0 a la esquerre
        cor_text = str(samples.loc [i,"Coments"])
        name3= "Sediment corers acquired on board R/V " + vessel + " with a " +cor_text+ " during the "+cruise_name+ " cruise."
        fila=fila+1
    ...
    posList.text = cruise_name + " corer data"
    tree.write(cdi_global)
    
    #afegir ABSTRACT
    tree = etree.parse(cdi_global)
    posList = tree.xpath("//gco:CharacterString[contains(text(), 'new_ABSTRACT')]", namespaces=namespace)[0]
    posList.text = str(total_lines) +" sediment corers acquired on board R/V "+ vessel + " during the " +cruise_name+" cruise."
